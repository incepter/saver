            ...
            <nav className="-mb-px flex space-x-2 overflow-x-auto">
              {folder.sections.map((section) => (
                ...
                >
                  ...
                  <div className="absolute top-0 right-0 flex opacity-0 group-hover:opacity-100 focus-within:opacity-100">
                    <button
                      className="ml-1 text-blue-500 hover:text-blue-600"
                      onClick={(e) => {
                        e.stopPropagation();
                        if (activeSection === section.id) {
                          const sectionItem = document.getElementById(`section-${section.id}`);
                          if (sectionItem) {
                            const editButton = sectionItem.querySelector('button[title="Rename section"]');
                            if (editButton) {
